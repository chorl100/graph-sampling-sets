...
class NeighborhoodRegression:
    """
    Class that implements (graph) neighborhood inference by applying linear or logistic regression.
    Code from https://github.com/stallmo/graph_inference_learning
    """
    ...
    def _infer_binary_signals_graph(self, C, exclude_same_category_col=None, only_same_category_col=None, **kwargs):
        ...
        for i in range(self.signals_data.shape[0]):
            ...
            # construct train_df with all independent variables and add fake observation if necessary
